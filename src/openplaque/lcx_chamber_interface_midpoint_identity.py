...
def _consensus_prefix(paths, step=.25, tol=.60, sustain=4):
    paths=_orient(paths); minlen=min(_arc(p)[-1] for p in paths); q=np.arange(0,minlen+1e-9,step)
    s=np.stack([_interp(p,q) for p in paths],axis=0); centroid=np.median(s,axis=0)
    dev=np.max(np.linalg.norm(s-centroid[None,:,:],axis=2),axis=0); first_bad=None
    start=max(1,int(round(5.0/step)))
    for i in range(start,max(start,len(q)-sustain+1)):
        if np.all(dev[i:i+sustain]>tol): first_bad=i; break
...
def _truncation(long_path, short_path, step=.25):
    long_path,short_path=_orient([long_path,short_path]); L,S=_arc(long_path)[-1],_arc(short_path)[-1]
    q=np.arange(0,S+1e-9,step)
    if q[-1] < S-1e-6: q=np.r_[q,S]
    d=np.linalg.norm(_interp(long_path,q)-_interp(short_path,q),axis=1)
    return {'C6_length_mm':float(L),'C9_length_mm':float(S),'C6_extension_beyond_C9_mm':float(L-S),
...
def _surface_tree(path, max_points=300000):
    im=sitk.ReadImage(str(_req(path))); surf=sitk.LabelContour(sitk.Cast(im>0,sitk.sitkUInt8),False)
    a=sitk.GetArrayViewFromImage(surf); z=np.argwhere(a>0)
    ...
    del z,a,surf,im; gc.collect(); return cKDTree(xyz)


def _mask_tree(path, max_points=350000):
    im=sitk.ReadImage(str(_req(path))); a=sitk.GetArrayViewFromImage(im); z=np.argwhere(a>0)
    if not len(z): raise ValueError(f'No foreground in {path}')
    if len(z)>max_points: z=z[::int(math.ceil(len(z)/max_points))]
    xyz=_img_zyx_to_xyz(im,z).astype(np.float32,copy=False)
    del z,a,im; gc.collect(); return cKDTree(xyz)


def _pair_interface(anchor, atrium_tree, ventricle_tree, radius=18.0):
    anchor=np.asarray(anchor,float); last=None
    for r in (radius,22.0,28.0,35.0):
        ia=atrium_tree.query_ball_point(anchor,r); iv=ventricle_tree.query_ball_point(anchor,r)
        if len(ia)<20 or len(iv)<20:
            last=(r,len(ia),len(iv),0); continue
        A=np.asarray(atrium_tree.data[np.asarray(ia,dtype=int)],float); V=np.asarray(ventricle_tree.data[np.asarray(iv,dtype=int)],float)
        vtree=cKDTree(V); atree=cKDTree(A)
        d_av,jv=vtree.query(A); d_va,ja=atree.query(V)
        mids=[]; sep=[]
        for cutoff in (5.0,7.5,10.0,12.5,15.0):
            ...
            if len(mids)>=30:
                P=np.asarray(mids,float); S=np.asarray(sep,float)
                keep=np.linalg.norm(P-anchor[None,:],axis=1)<=r; P,S=P[keep],S[keep]
                if len(P)>=30:
                    key=np.round(P/.6).astype(int); _,ix=np.unique(key,axis=0,return_index=True); P,S=P[np.sort(ix)],S[np.sort(ix)]
                    if len(P)>=20:
                        return cKDTree(P.astype(np.float32,copy=False)), {'anchor_lps_mm':[float(x) for x in anchor],'used_radius_mm':float(r),'pair_separation_cut_mm':float(cutoff),'n_atrial_surface_points':int(len(A)),'n_ventricular_surface_points':int(len(V)),'n_interface_midpoints':int(len(P)),'median_pair_separation_mm':float(np.median(S)),'p90_pair_separation_mm':float(np.percentile(S,90)),'selection_mode':'mutual_symmetric_nearest_neighbors'}
...
def _local_interface_tangent(tree,x,k=30):
    kk=min(k,len(tree.data)); _,idx=tree.query(np.asarray(x,float),k=kk)
    hood=np.atleast_2d(np.asarray(tree.data[idx],float)); hood=hood-hood.mean(axis=0)
    if len(hood)<3: return np.zeros(3),0.0
    _,s,vh=np.linalg.svd(hood,full_matrices=False); v=vh[0]; n=np.linalg.norm(v)
...
def _score(path,interface_tree,max_mm=8.0):
    p,q=_resample(path,.25); keep=q<=min(q[-1],max_mm)+1e-9; p,q=p[keep],q[keep]
    d,_=interface_tree.query(p); t=_tangents(p); aligns=[]; anis=[]
    for x,v in zip(p,t):
        r,a=_local_interface_tangent(interface_tree,x); aligns.append(abs(float(np.dot(v,r))) if np.linalg.norm(r)>0 else 0.0); anis.append(a)
    ...
    return {'interface_identity_score':score,'median_interface_distance_mm':med_d,'p90_interface_distance_mm':float(np.percentile(d,90)),'endpoint_interface_distance_mm':end_d,'interface_distance_slope_mm_per_mm':sl,'interface_retention_score':retention,'median_interface_tangent_alignment':align,'median_local_interface_anisotropy':an,'profile_arc_mm':q,'profile_interface_distance_mm':d,'profile_alignment':aligns,'points':p}
...
def _hu(src,g,path):
    p,_=_resample(path,.25); h=_sample(src,g,p); return float(np.mean((h>=120)&(h<=1200))),float(np.median(h))
...
def synthetic_interface_self_test():
    # Long enough that the production 0.6-mm midpoint deduplication still leaves >=20 unique interface points.
    ...
    tree,meta=_pair_interface(np.array([10.,0.,0.]),at,vt,12.0)
    good=np.column_stack([x,np.ones_like(x)*.4,np.zeros_like(x)])
    bad=np.column_stack([x,np.ones_like(x)*(2+.25*x),np.zeros_like(x)])
    g=_score(good,tree,8.0); b=_score(bad,tree,8.0)
    assert meta['n_interface_midpoints']>=20
...
def run(drive_root='/content/drive/MyDrive/OpenPlaque', output_dir=None):
    ...
    leaves=_orient([_load_path(root/p,g) for p in LEAF_FILES]); ranking=pd.read_csv(root/PRIOR_RANKING); mapping={int(ranking.iloc[i]['candidate_id']):leaves[i] for i in range(5)}
    for cid in (6,7,9):
        if cid not in mapping: raise RuntimeError(f'Missing source candidate C{cid}')
    c6,c7,c9=mapping[6],mapping[7],mapping[9]; _,f2q,_,_=_consensus_prefix([c6,c7,c9],.25,.60,4); split_arc=float(f2q[-1]); trunc=_truncation(c6,c9); topo_pass=bool(trunc['truncation_gate_pass'] and 19.0<=split_arc<=21.0)
    print('Building sparse chamber and coronary trees...')
    la=_surface_tree(root/LA); lv=_surface_tree(root/LV); ra=_surface_tree(root/RA); rv=_surface_tree(root/RV); cur=_mask_tree(root/COR_CURRENT); leg=_mask_tree(root/COR_LEGACY)
    left_anchor=_interp(c6,[split_arc])[0]; left_interface,left_meta=_pair_interface(left_anchor,la,lv,18.0)
    rca_total=_arc(rca)[-1]; rca_anchor_arc=min(30.0,max(8.0,.58*rca_total)); right_anchor=_interp(rca,[rca_anchor_arc])[0]; right_interface,right_meta=_pair_interface(right_anchor,ra,rv,18.0)
    ...
    rca_seg=_slice_from_arc(rca,max(0.0,rca_anchor_arc-3.0),10.0); rca_m=_score(rca_seg,right_interface,10.0)
    lad_rs,lad_q=_resample(lad,.25); j=int(np.argmin(np.linalg.norm(lad_rs-left_anchor[None,:],axis=1))); lad_start=max(0.0,float(lad_q[j])-3.0); lad_seg=_slice_from_arc(lad,lad_start,6.0); lad_m=_score(lad_seg,left_interface,6.0)
    control_margin=float(rca_m['interface_identity_score']-lad_m['interface_identity_score']); control_pass=bool(rca_m['median_interface_distance_mm']<=8.0 and rca_m['median_interface_tangent_alignment']>=.45 and control_margin>=.08)
    controls={'RCA_right_interface_score':rca_m['interface_identity_score'],'RCA_median_interface_distance_mm':rca_m['median_interface_distance_mm'],'RCA_median_tangent_alignment':rca_m['median_interface_tangent_alignment'],'LAD_left_negative_score':lad_m['interface_identity_score'],'LAD_median_interface_distance_mm':lad_m['median_interface_distance_mm'],'LAD_median_tangent_alignment':lad_m['median_interface_tangent_alignment'],'control_margin':control_margin,'control_pass':control_pass}
    _write_json(out/'chamber_interface_controls.json',controls)
    c6_post=_slice_from_arc(c6,split_arc,8.0); c7_post=_slice_from_arc(c7,split_arc,8.0); m6=_score(c6_post,left_interface,8.0); m7=_score(c7_post,left_interface,8.0)
    ...
    rows=[]
    for cid,m in [(6,m6),(7,m7)]: rows.append({k:v for k,v in {'source_candidate_id':cid,**m}.items() if not isinstance(v,np.ndarray)})
    ...
    plt.figure(figsize=(6,4)); plt.bar(['C6','C7'],[m6['interface_identity_score'],m7['interface_identity_score']]); plt.ylabel('interface identity score'); plt.title(f'Chamber-interface midpoint | control pass={control_pass}'); plt.tight_layout(); plt.savefig(out/'01_chamber_interface_scores.png',dpi=180); plt.close()
    plt.figure(figsize=(7,4)); plt.plot(m6['profile_arc_mm'],m6['profile_interface_distance_mm'],label='C6'); plt.plot(m7['profile_arc_mm'],m7['profile_interface_distance_mm'],label='C7'); plt.xlabel('downstream arc (mm)'); plt.ylabel('distance to chamber interface (mm)'); plt.legend(); plt.tight_layout(); plt.savefig(out/'02_chamber_interface_distance_profiles.png',dpi=180); plt.close()
    plt.figure(figsize=(7,4)); plt.plot(m6['profile_arc_mm'],m6['profile_alignment'],label='C6'); plt.plot(m7['profile_arc_mm'],m7['profile_alignment'],label='C7'); plt.xlabel('downstream arc (mm)'); plt.ylabel('|tangent · local interface PCA tangent|'); plt.ylim(0,1.05); plt.legend(); plt.tight_layout(); plt.savefig(out/'03_chamber_interface_alignment_profiles.png',dpi=180); plt.close()
    fig=plt.figure(figsize=(10,8)); ax=fig.add_subplot(111,projection='3d'); lp=np.asarray(left_interface.data); ax.scatter(lp[:,0],lp[:,1],lp[:,2],s=5,alpha=.25,label='LA-LV interface midpoints'); ax.plot(c6_post[:,0],c6_post[:,1],c6_post[:,2],lw=3,label='C6'); ax.plot(c7_post[:,0],c7_post[:,1],c7_post[:,2],lw=3,label='C7'); ax.scatter([left_anchor[0]],[left_anchor[1]],[left_anchor[2]],s=55,label='F2 split'); ax.legend(); ax.set_title('Direct chamber-interface midpoint geometry'); plt.tight_layout(); plt.savefig(out/'04_chamber_interface_geometry.png',dpi=180); plt.close()
    ...
    summary={'status':status,'algorithm':ALGORITHM,'baseline_commit':BASELINE,'master_status':master.get('status'),'LCX_master_status':'UNRESOLVED','topology_prerequisite_pass':topo_pass,'split_arc_mm':split_arc,'C6_C9_truncation':trunc,'chamber_interface_controls':controls,'C6_metrics':{k:v for k,v in m6.items() if not isinstance(v,np.ndarray)},'C7_metrics':{k:v for k,v in m7.items() if not isinstance(v,np.ndarray)},'decision':decision,'myocardium_used_in_interface':False,'template_similarity_used_in_decision':False,'prior_AV_groove_score_used_in_decision':False,'scientific_boundary':'A positive result nominates an LCX-like chamber-interface-following daughter and an OM-like departing daughter. It does not establish clinical identity; LM remains unresolved.'}
    _write_json(out/'summary.json',summary); report=out/'OPENPLAQUE_LCX_CHAMBER_INTERFACE_MIDPOINT_REPORT.html'; report.write_text(f"<html><body><h1>OpenPlaque LCX Chamber Interface Midpoint</h1><p><b>Status:</b> {status}</p><p>Control pass: {control_pass}; margin {control_margin:.3f}.</p><p>C6 score {m6['interface_identity_score']:.3f}; C7 score {m7['interface_identity_score']:.3f}; winner C{win_id}; identity gate {identity_gate}.</p></body></html>",encoding='utf-8'); _write_json(out/'run_state.json',{'status':'COMPLETE','scientific_status':status,'algorithm':ALGORITHM,'baseline_commit':BASELINE}); zpath=out/'OPENPLAQUE_LCX_CHAMBER_INTERFACE_MIDPOINT_REPORT_BACK.zip'
