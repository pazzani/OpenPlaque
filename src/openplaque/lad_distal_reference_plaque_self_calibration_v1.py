...
ALGORITHM = "lad-distal-reference-plaque-self-calibration-v1.0"
OUTPUT_DIRNAME = "LAD_Distal_Reference_Plaque_Self_Calibration_v1"
...
FRAME_SUMMARY = Path("Cross_Vessel_Lumen_Frame_Refinement_Plaque_Rescue_v1/summary.json")
...
# Prespecified, label-blind distal-extension reference region.
# The extension is represented with negative frozen-LAD arc. We exclude the
# final 3 mm adjacent to the frozen-LAD join.
LAD_REFERENCE_ARC = (-15.0, -3.0)
LAD_EVAL_ARC = (0.0, 25.0)
...
MIN_REFERENCE_STATIONS = 18
MIN_REFERENCE_QC = 0.95
MIN_FROZEN_QC = 0.95
MIN_POSITIVE_BINS = 4
MIN_NEGATIVE_BINS = 5
MIN_STRICT_BINS = 2
...
STATUS_GEOMETRY_FAIL = "LAD_DISTAL_REFERENCE_SELF_CALIBRATION_GEOMETRY_FAILED"
STATUS_SPEC_FAIL = "LAD_DISTAL_REFERENCE_SELF_CALIBRATION_SPECIFICITY_FAILED"
STATUS_PASS = "LAD_DISTAL_REFERENCE_SELF_CALIBRATION_DEVELOPMENTAL_PASS"
...
def _join_frozen_and_extension(frozen, extension, max_join_mm=1.5):
    ...
    if d0 > max_join_mm:
        raise RuntimeError(f"Distal extension does not join frozen LAD arc-0 endpoint: {d0:.3f} mm")
...
def _quantify_lad(geom, src, combined, ext_len):
    centers, arcs = _resample(combined)
    ...
    for i,(c,tr,ts,a,ds) in enumerate(zip(centers,raw_t,smooth_t,arcs,weights)):
        ...
        frozen_arc = float(a - ext_len)
        region = "distal_extension" if frozen_arc < -0.25 else "frozen_lad"
        for shell in SHELLS:
...
def _huber_fit(X, y, k=HUBER_K, n_iter=40):
    ...
    if len(y) < X.shape[1] + 4:
        raise RuntimeError("Insufficient finite LAD distal-reference stations")
...
def _fit_reference_models(stations):
    models=[]
    for shell in SHELLS:
        d=stations[np.isclose(stations.shell_thickness_mm,shell) & stations.station_qc_pass.astype(bool)].copy()
        ref=(d.frozen_arc_mm>=LAD_REFERENCE_ARC[0]) & (d.frozen_arc_mm<LAD_REFERENCE_ARC[1])
        if int(ref.sum())<MIN_REFERENCE_STATIONS:
            raise RuntimeError(f"Too few LAD distal-reference stations for shell {shell}: {int(ref.sum())}")
        X=_design(d)
        for c in COMPONENTS:
...
def _apply_models(stations,models):
    ...
    for shell in SHELLS:
        ...
        for c in COMPONENTS:
            ...
            if len(m)!=1:
                raise RuntimeError(f"Missing model for shell {shell}, {c}")
            m=m.iloc[0]
            ...
            excess[~d.station_qc_pass.to_numpy(bool)]=np.nan
            stem=c.replace("_mm3","")
...
def _profile_1mm(scored,shell):
    d=scored[np.isclose(scored.shell_thickness_mm,shell) & scored.station_qc_pass.astype(bool)].copy()
    d["arc_start_mm"]=np.floor(d.frozen_arc_mm.to_numpy(float)).astype(float)
    rows=[]
    ...
    for a,g in d.groupby("arc_start_mm",sort=True):
        r={
            ...
            "region":"distal_reference" if LAD_REFERENCE_ARC[0] <= a < LAD_REFERENCE_ARC[1] else (
                "distal_extension" if a<0 else "frozen_lad"
            ),
...
def _shell_consistency(profiles):
    ...
    for shell,p in profiles.items():
        ...
        ok=(nom.index>=0)&nom.notna()&y.notna()
...
def _specificity_pass(m,min_rho):
    return bool(
        ...
        and np.isfinite(min_rho) and min_rho>=MIN_CROSS_SHELL_SPEARMAN
    )
...
def _plot_profile(profile,matched,out):
    fig,ax=plt.subplots(figsize=(12,6))
    ax.plot(profile.arc_start_mm+.5,profile.excess_total_plaque_proxy_mm3,linewidth=2,label="LAD self-calibrated excess")
    ax.axvspan(LAD_REFERENCE_ARC[0],LAD_REFERENCE_ARC[1],alpha=.07,label="distal reference")
    ax.axvline(0,linestyle="--",linewidth=1)
    ax.set_xlabel("LAD arc relative to frozen distal endpoint (mm)")
    ax.set_ylabel("Excess proxy per 1-mm bin (mm³)")
    ax2=ax.twinx()
    if len(matched):
        ax2.step(matched.arc_start_mm+.5,matched.mapped_native_voxels_vote_ge3.fillna(0),where="mid",alpha=.45,label="prior 3+/5 vote voxels")
    ax2.set_ylabel("Prior ensemble 3+/5 vote voxels")
    lines=ax.get_lines()+ax2.get_lines()
    ax.legend(lines,[l.get_label() for l in lines],loc="upper right")
    ax.set_title("LAD distal-reference self-calibration")
    fig.tight_layout()
...
def _plot_shells(profiles,out):
    ...
    for shell,p in profiles.items():
        ax.plot(p.arc_start_mm+.5,p.excess_total_plaque_proxy_mm3,label=f"{shell:.2f} mm")
    ax.axvspan(LAD_REFERENCE_ARC[0],LAD_REFERENCE_ARC[1],alpha=.06)
    ax.set_xlabel("LAD arc relative to frozen distal endpoint (mm)")
    ax.set_ylabel("Excess proxy per 1-mm bin (mm³)")
    ax.set_title("LAD shell-sensitivity")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out,dpi=170)
    plt.close(fig)
...
def _plot_qc(stations,out):
    d=stations[np.isclose(stations.shell_thickness_mm,NOMINAL_SHELL)].copy()
    fig,ax=plt.subplots(figsize=(12,5.5))
    ax.plot(d.frozen_arc_mm,d.raw_lumen_axis_proxy,label="raw axis proxy",alpha=.6)
    ax.plot(d.frozen_arc_mm,d.lumen_axis_proxy,label="refined axis proxy")
    ax.axhline(2.5,linestyle="--",linewidth=1)
    ax.axvspan(LAD_REFERENCE_ARC[0],LAD_REFERENCE_ARC[1],alpha=.06)
    ax2=ax.twinx()
    ax2.plot(d.frozen_arc_mm,d.center_refine_offset_mm,alpha=.4,label="center shift")
    ax.set_xlabel("LAD arc relative to frozen distal endpoint (mm)")
    ax.set_ylabel("p90/p10 lumen axis proxy")
    ax2.set_ylabel("center refinement (mm)")
    lines=ax.get_lines()+ax2.get_lines()
    ax.legend(lines,[l.get_label() for l in lines],loc="upper right")
    ax.set_title("LAD source-plane geometry QC")
    fig.tight_layout()
    fig.savefig(out,dpi=170)
    plt.close(fig)


def _plot_planes(geom,src,centers,raw_t,smooth_t,stations,out):
    d=stations[np.isclose(stations.shell_thickness_mm,NOMINAL_SHELL)].copy()
    # Automatically choose representative reference/evaluation stations plus worst refined axis.
    candidates=[]
    for target in (-12,-6,2,8,18,24):
        idx=int(np.argmin(np.abs(d.frozen_arc_mm.to_numpy(float)-target)))
        candidates.append(int(d.iloc[idx].station_index))
    worst=int(d.sort_values("lumen_axis_proxy",ascending=False).iloc[0].station_index)
    candidates.append(worst)
    ids=list(dict.fromkeys(candidates))[:6]
    fig,axes=plt.subplots(2,3,figsize=(12,8))
    axes=np.asarray(axes).ravel()
    for ax in axes: ax.axis("off")
    for ax,i in zip(axes,ids):
        row=d[d.station_index==i].iloc[0]
        c=np.array([row.refined_center_lps_x_mm,row.refined_center_lps_y_mm,row.refined_center_lps_z_mm],float)
        im,q=_plane_image(geom,src,c,smooth_t[i])
        ax.imshow(im,cmap="gray",vmin=-100,vmax=800,extent=[q[0],q[-1],q[-1],q[0]])
        ax.scatter([0],[0],s=10)
        ax.set_title(f"{row.frozen_arc_mm:.1f} mm | axis {row.lumen_axis_proxy:.2f} | shift {row.center_refine_offset_mm:.2f}")
        ax.set_xticks([]);ax.set_yticks([]);ax.axis("on")
    fig.suptitle("LAD refined source-CCTA planes")
    fig.tight_layout()
...
def synthetic_self_test():
    ...
    b=np.array([[-2.,0.,0.],[-1.,0.,0.],[0.,0.,0.]])
    combined,ext_len,gap=_join_frozen_and_extension(a,b)
    assert abs(ext_len-2.0)<1e-9
    assert gap<1e-9
    assert len(combined)==5
    assert _auc([0,0,1,1],[.1,.2,.8,.9])==1.0
    return {"ok":True,"extension_length_mm":ext_len,"join_gap_mm":gap}
...
def run(drive_root="/content/drive/MyDrive/OpenPlaque",output_dir=None):
    ...
    prev=_read_json(root/PREV_LAD_SUMMARY) if (root/PREV_LAD_SUMMARY).exists() else {}
    frame=_read_json(root/FRAME_SUMMARY) if (root/FRAME_SUMMARY).exists() else {}
    ...
    if bidir.get("status")!=EXPECTED_BIDIR_STATUS:
        raise RuntimeError(f"Distal LAD bidirectional prerequisite failed: {bidir.get('status')}")
    ...
    if ext_len < abs(LAD_REFERENCE_ARC[0]) - .5:
        raise RuntimeError(f"Extension too short for prespecified reference region: {ext_len:.3f} mm")

    stations,centers,raw_t,smooth_t=_quantify_lad(geom,src,combined,ext_len)
    stations.to_csv(out/"LAD_selfcal_source_space_stations.csv",index=False)

    nominal=stations[np.isclose(stations.shell_thickness_mm,NOMINAL_SHELL)].copy()
    refmask=(nominal.frozen_arc_mm>=LAD_REFERENCE_ARC[0])&(nominal.frozen_arc_mm<LAD_REFERENCE_ARC[1])
    frozenmask=(nominal.frozen_arc_mm>=0)&(nominal.frozen_arc_mm<=frozen_len+.25)
    reference_qc=float(nominal.loc[refmask,"station_qc_pass"].mean())
    frozen_qc=float(nominal.loc[frozenmask,"station_qc_pass"].mean())
    raw_reference_qc=float(nominal.loc[refmask,"raw_station_qc_pass"].mean())
    raw_frozen_qc=float(nominal.loc[frozenmask,"raw_station_qc_pass"].mean())

    geometry_pass=bool(
        int(refmask.sum())>=MIN_REFERENCE_STATIONS
        and reference_qc>=MIN_REFERENCE_QC
        and frozen_qc>=MIN_FROZEN_QC
    )

    models=_fit_reference_models(stations)
    models.to_csv(out/"LAD_distal_reference_models.csv",index=False)
    scored=_apply_models(stations,models)
    scored.to_csv(out/"LAD_selfcal_excess_stations.csv",index=False)

    profiles={}
    for shell in SHELLS:
        profiles[shell]=_profile_1mm(scored,shell)
    all_profiles=pd.concat(profiles.values(),ignore_index=True)
    all_profiles.to_csv(out/"LAD_selfcal_excess_profiles_all_shells.csv",index=False)
    nominal_profile=profiles[NOMINAL_SHELL]
    nominal_profile.to_csv(out/"LAD_selfcal_excess_profile_nominal.csv",index=False)

    prior=pd.read_csv(_req(root/LAD_PRIOR))
    matched,metrics=_developmental_metrics(nominal_profile,prior,frozen_len)
    matched.to_csv(out/"LAD_selfcal_developmental_validation_bins.csv",index=False)

    consistency,min_rho=_shell_consistency(profiles)
    consistency.to_csv(out/"LAD_selfcal_shell_consistency.csv",index=False)
    specificity_pass=_specificity_pass(metrics,min_rho)
    ...
    frozen_profile=nominal_profile[(nominal_profile.arc_start_mm>=0)&(nominal_profile.arc_start_mm<frozen_len)].copy()
    ref_profile=nominal_profile[(nominal_profile.arc_start_mm>=LAD_REFERENCE_ARC[0])&(nominal_profile.arc_start_mm<LAD_REFERENCE_ARC[1])].copy()
    compcols=[f"excess_{c.replace('_mm3','')}_mm3" for c in COMPONENTS]
    frozen_totals={c:float(frozen_profile[c].sum()) for c in compcols+["excess_total_plaque_proxy_mm3"]}
    reference_totals={c:float(ref_profile[c].sum()) for c in compcols+["excess_total_plaque_proxy_mm3"]}

    _plot_profile(nominal_profile,matched,out/"01_LAD_selfcal_excess_vs_prior.png")
    _plot_shells(profiles,out/"02_LAD_selfcal_shell_sensitivity.png")
    _plot_qc(stations,out/"03_LAD_selfcal_geometry_QC.png")
    _plot_planes(geom,src,centers,raw_t,smooth_t,stations,out/"04_LAD_selfcal_source_planes.png")
    ...
    summary={
        ...
        "frozen_lad_length_mm":frozen_len,
        "distal_extension_length_mm":ext_len,
        "combined_length_mm":float(_arc(combined)[-1]),
        "join_gap_mm":join_gap,
        "source_voxel_volume_mm3":voxel_volume,
        "reference_region_frozen_arc_mm":list(LAD_REFERENCE_ARC),
        "reference_selection":(
            "Prespecified distal-extension segment selected by anatomy only. The final 3 mm adjacent to the frozen-LAD join are excluded. "
            "Plaque-vote labels are not used to fit the reference model."
        ),
        "geometry":{
            "reference_station_count":int(refmask.sum()),
            "reference_qc_fraction":reference_qc,
            "frozen_qc_fraction":frozen_qc,
            "raw_reference_qc_fraction":raw_reference_qc,
            "raw_frozen_qc_fraction":raw_frozen_qc,
            "geometry_gate_pass":geometry_pass,
            "median_center_refinement_mm":float(nominal.center_refine_offset_mm.median()),
            "max_center_refinement_mm":float(nominal.center_refine_offset_mm.max()),
        },
        "developmental_specificity":metrics,
        "min_cross_shell_profile_spearman":min_rho,
        "specificity_gate_pass":specificity_pass,
        "nominal_frozen_lad_excess_totals_mm3":frozen_totals,
        "nominal_distal_reference_excess_totals_mm3":reference_totals,
        "previous_rca_transfer_status":prev.get("status"),
        "previous_frame_refinement_status":frame.get("status"),
        "is_independent_validation":False,
        "is_validated_clinical_tpv":False,
        "scientific_boundary":(
            "This is a developmental LAD-specific self-calibration. The normal-wall model is fitted only in a prespecified, label-blind distal "
            "extension reference segment, then evaluated on the frozen LAD against plaque-vote labels that have been inspected in prior development. "
            "A pass therefore supports a research LAD proxy but is not independent validation or clinical TPV. Frozen master anatomy is unchanged."
        ),
    }
    ...
    report=out/"OPENPLAQUE_LAD_DISTAL_REFERENCE_PLAQUE_SELF_CALIBRATION_REPORT.html"
    report.write_text(
        "<html><body><h1>OpenPlaque LAD Distal-Reference Plaque Self-Calibration v1</h1>"
        f"<p><b>Status:</b> {status}</p>"
        f"<p>Frozen LAD length: {frozen_len:.3f} mm; validated distal extension: {ext_len:.3f} mm.</p>"
        f"<p>Reference QC: {reference_qc:.3f}; frozen-LAD QC: {frozen_qc:.3f}; cross-shell Spearman: {min_rho:.3f}.</p>"
        f"<p>Majority AUROC: {metrics.get('majority_vs_vote_free_auc',float('nan')):.3f}; "
        f"strict AUROC: {metrics.get('strict5_vs_vote_free_auc',float('nan')):.3f}; "
        f"positive/negative ratio: {metrics.get('positive_negative_median_ratio',float('nan')):.3f}.</p>"
        f"<p>Nominal frozen-LAD excess proxy: {frozen_totals['excess_total_plaque_proxy_mm3']:.3f} mm³.</p>"
        "<p><b>Boundary:</b> developmental LAD-specific self-calibration; plaque labels are not used to fit the distal reference model, "
        "but the frozen-LAD evaluation labels were previously inspected. Not independent validation and not clinical TPV.</p>"
        "<h2>Geometry</h2><pre>"+json.dumps(summary["geometry"],indent=2,default=str)+"</pre>"
        "<h2>Developmental specificity</h2><pre>"+json.dumps(metrics,indent=2,default=str)+"</pre>"
        "<h2>Frozen-LAD totals</h2><pre>"+json.dumps(frozen_totals,indent=2,default=str)+"</pre>"
        '<img src="01_LAD_selfcal_excess_vs_prior.png" style="max-width:100%">'
        '<img src="02_LAD_selfcal_shell_sensitivity.png" style="max-width:100%">'
        '<img src="03_LAD_selfcal_geometry_QC.png" style="max-width:100%">'
        '<img src="04_LAD_selfcal_source_planes.png" style="max-width:100%">'
        "</body></html>",
        encoding="utf-8"
    )
    ...
    zpath=out/"OPENPLAQUE_LAD_DISTAL_REFERENCE_PLAQUE_SELF_CALIBRATION_RESULTS.zip"
