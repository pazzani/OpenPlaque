...
ALGORITHM = "series6-left-coronary-origin-validation-v1.0"
OUTPUT_DIRNAME = "Series6_Left_Coronary_Origin_Validation_v1"
...
STATUS_PRESENT = "SERIES6_DISTINCT_LEFT_OSTIUM_PRESENT"
STATUS_ABSENT = "SERIES6_NO_DISTINCT_LEFT_OSTIUM"
STATUS_RCA_FAIL = "SERIES6_RCA_CONTROL_FAILED"
STATUS_REG_FAIL = "SERIES6_REGISTRATION_FAILED"

SERIES7_FOLDER_NAME = "32210"
...
def prepare(
    ...
    dicom_root=None,
    local_workdir="/content/openplaque_series6",
    output_dir=None,
):
    root = Path(drive_root)
    ...
    series7_folder = _req(Path(dicom_root) / SERIES7_FOLDER_NAME)
    series6_folder = _req(Path(dicom_root) / SERIES6_FOLDER_NAME)

    inv = scan_dicom_series(dicom_root)
    inv.to_csv(out / "series_inventory.csv", index=False)

    s7 = inv[(inv.series_number == SERIES7_NUMBER) & inv.folder.eq(str(series7_folder))]
    s6 = inv[(inv.series_number == SERIES6_NUMBER) & inv.folder.eq(str(series6_folder))]
    if len(s7) != 1:
        raise RuntimeError(f"Expected exactly one Series 7 row for {series7_folder}, found {len(s7)}")
    if len(s6) != 1:
        raise RuntimeError(f"Expected exactly one Series 6 row for {series6_folder}, found {len(s6)}")
    s7 = s7.iloc[0]
    s6 = s6.iloc[0]

    # Hard scientific identity checks: only like-for-like high-resolution coronary source series.
    if "bestdiast" not in str(s7.series_description).lower():
        raise RuntimeError(f"Series 7 description mismatch: {s7.series_description}")
    if "bestsyst" not in str(s6.series_description).lower():
        raise RuntimeError(f"Series 6 description mismatch: {s6.series_description}")
    for row, name in ((s7, "Series 7"), (s6, "Series 6")):
        if int(row.rows) != 512 or int(row.cols) != 512 or int(row.n_files) != 524:
            raise RuntimeError(f"{name} unexpected matrix/slice count: {row.rows}x{row.cols}x{row.n_files}")
        if not np.isfinite(row.pixel_spacing_x_mm) or float(row.pixel_spacing_x_mm) > 0.40:
            raise RuntimeError(f"{name} unexpected in-plane spacing: {row.pixel_spacing_x_mm}")
        if not np.isfinite(row.slice_spacing_mm) or float(row.slice_spacing_mm) > 0.40:
            raise RuntimeError(f"{name} unexpected slice spacing: {row.slice_spacing_mm}")

    series6_img, series6_arr, series6_geom = load_dicom_series(series6_folder)
    bright = _path_brightness(series6_geom, series6_arr, [lad, rca])
    if bright["path_coverage_fraction"] < 0.95:
        raise RuntimeError(f"Series 6 frozen-path coverage too low: {bright}")
    if not np.isfinite(bright["path_median_hu"]) or bright["path_median_hu"] < 150:
        raise RuntimeError(f"Series 6 does not show adequate contrast coronary signal: {bright}")

    input_dir = local / "model_inputs"
    input_dir.mkdir(parents=True, exist_ok=True)
    ref_id = "series7_reference"
    alt_id = "series6_alternate"
    _write_mha(ref_img, input_dir / f"{ref_id}.img.mha")
    _write_mha(series6_img, input_dir / f"{alt_id}.img.mha")

    tx, reg_info = register_translation(ref_img, series6_img, root_center)
    tfm = local / "series6_to_series7.tfm"
    sitk.WriteTransform(tx, str(tfm))
    pd.DataFrame([{
        "scan_id": alt_id,
        "series_number": SERIES6_NUMBER,
        "series_description": str(s6.series_description),
        "path_coverage_fraction": bright["path_coverage_fraction"],
        "path_median_hu": bright["path_median_hu"],
        "path_p10_hu": bright["path_p10_hu"],
        **reg_info,
    }]).to_csv(out / "registration_summary.csv", index=False)
    ...
    prep = {
        ...
        "series7": {
            "scan_id": ref_id,
            "series_number": 7,
            "series_description": str(s7.series_description),
            "folder": str(series7_folder),
            "input_file": str(input_dir / f"{ref_id}.img.mha"),
        },
        "series6": {
            "scan_id": alt_id,
            "series_number": 6,
            "series_description": str(s6.series_description),
            "folder": str(series6_folder),
            "input_file": str(input_dir / f"{alt_id}.img.mha"),
    ...
    _write_json(out / "input_provenance.json", {
        "dicom_root": str(dicom_root),
        "series7_folder": str(series7_folder),
        "series6_folder": str(series6_folder),
    ...
    _write_json(out / "run_state.json", {
        ...
        "algorithm": ALGORITHM,
        "baseline": BASELINE,
        "series6_registration_pass": bool(reg_info["registration_pass"]),
    })
    return prep
...
def analyze(
    ...
    records = [
        {
            "scan_id": prep["series7"]["scan_id"],
            "role": "reference",
            "series_number": 7,
            "series_description": prep["series7"]["series_description"],
            "transform_file": None,
            "registration": {"registration_pass": True},
        },
        {
            "scan_id": prep["series6"]["scan_id"],
            "role": "alternate",
            "series_number": 6,
            "series_description": prep["series6"]["series_description"],
            "transform_file": prep["series6"]["transform_file"],
            "registration": prep["series6"]["registration"],
        },
    ]
    ...
    for rec in records:
        ...
        tx = sitk.Transform() if rec["role"] == "reference" else sitk.ReadTransform(str(rec["transform_file"]))
        pred = _mask_on_reference(pred_path, ref_img, tx)
        sl, offset, labels, ncomp, clab, ncontacts = _sparse_components(pred, geom, aorta)
        tree, lab_near = _mask_points_tree(pred, geom, labels, offset, sl)

        supports = {
            k: _path_support_tree(v, tree, lab_near)
            for k, v in {"LAD": lad, "RCA": rca, "C6": c6, "C7": c7}.items()
        }
        for path_name, s in supports.items():
            path_rows.append({"scan_id": sid, "role": rec["role"], "path": path_name, **s})

        cr = _contact_rows(clab, labels, offset, geom, rca_root, left_anchor, sid)
        ...
        reg_pass = bool(rec["registration"].get("registration_pass", False))
        gates = {
        ...
        for gate, val in gates.items():
            gate_rows.append({"scan_id": sid, "role": rec["role"], "gate": gate, "pass": bool(val)})
        ...
        if rec["role"] == "reference":
            ...
            moving = sitk.ReadImage(str(local / "model_inputs" / f"{sid}.img.mha"))
            ct_img = sitk.Resample(moving, ref_img, tx, sitk.sitkLinear, -1024.0, sitk.sitkFloat32)
            ct_ref = sitk.GetArrayFromImage(ct_img)

        _plot_root_qc(
            sid, ct_ref, pred, aorta, geom, left_anchor, rca_root,
            out / f"QC_{sid}_root_planes.png",
        )

    pd.DataFrame(path_rows).to_csv(out / "path_support.csv", index=False)
    ...
    pd.DataFrame(result_rows).to_csv(out / "series_decisions.csv", index=False)
    _plot_contacts(contact_df, out / "01_aortic_contact_cluster_distances.png")

    s6 = series_results["series6_alternate"]
    if not s6["registration"]["registration_pass"]:
        status = STATUS_REG_FAIL
    elif not (
        s6["gates"]["RCA_support_control"]
        and s6["gates"]["RCA_distinct_aortic_contact_control"]
    ):
        status = STATUS_RCA_FAIL
    elif s6["left_contact_positive"]:
    ...
    decision = {
        "status": status,
        "series6_registration_pass": bool(s6["registration"]["registration_pass"]),
        "series6_RCA_control_pass": bool(
            s6["gates"]["RCA_support_control"]
            and s6["gates"]["RCA_distinct_aortic_contact_control"]
        ),
        "series6_left_contact_positive": bool(s6["left_contact_positive"]),
        "series6_left_contact_candidates": s6["left_contact_candidates"],
        "clinical_LM_identity_established": False,
        "clinical_LCX_OM_identity_established": False,
        "master_anatomy_modified": False,
        "reopen_left_coronary_review": bool(status == STATUS_PRESENT),
    }
    ...
    summary = {
        ...
        "series7": series_results["series7_reference"],
        "series6": s6,
        "decision": decision,
        "scientific_boundary": (
            "Same-exam alternate cardiac phase evidence only. A positive result requires "
            "a distinct left aortic contact separated from the RCA ostium. Frozen master "
            "remains unchanged; clinical LM/LCX/OM identity is not automatically promoted."
        ),
    }
    ...
    report = out / "OPENPLAQUE_SERIES6_LEFT_CORONARY_ORIGIN_VALIDATION_V1_REPORT.html"
    report.write_text(
        "<html><body><h1>OpenPlaque Series 6 Left Coronary Origin Validation v1</h1>"
        f"<p><b>Status:</b> {status}</p>"
        "<p>Comparison: Series 7 BestDiast 63% vs Series 6 BestSyst 32%.</p>"
        "<p>Distinct left aortic contact is explicitly separated from the RCA ostium. "
        "Frozen master unchanged.</p>"
        "<h2>Series decisions</h2>" + pd.DataFrame(result_rows).to_html(index=False) +
        "<h2>Aortic-contact QC</h2>"
        "<img src='01_aortic_contact_cluster_distances.png' width='900'>"
        "<h2>Series 7 root planes</h2><img src='QC_series7_reference_root_planes.png' width='1000'>"
        "<h2>Series 6 root planes</h2><img src='QC_series6_alternate_root_planes.png' width='1000'>"
        "</body></html>",
        encoding="utf-8",
    )
    ...
    archive = out / "OPENPLAQUE_SERIES6_LEFT_CORONARY_ORIGIN_VALIDATION_V1_RESULTS.zip"
...
def synthetic_self_test():
    ...
    rca_contact = np.array([1.0, 0.0, 0.0])
    left_contact = np.array([18.5, 0.5, 0.0])
